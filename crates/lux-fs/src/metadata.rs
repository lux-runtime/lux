...
/// Timestamp wrapper for Lua
#[derive(Debug, Clone, Copy)]
pub struct FsTimestamp(i64);

impl IntoLua for FsTimestamp {
    fn into_lua(self, lua: &Lua) -> LuaResult<LuaValue> {
        self.0.into_lua(lua)
    }
}
...
fn system_time_to_timestamp(res: IoResult<SystemTime>) -> Option<FsTimestamp> {
    res.ok().and_then(|t| {
        t.duration_since(SystemTime::UNIX_EPOCH)
            .ok()
            .map(|d| FsTimestamp(d.as_secs() as i64))
    })
}
