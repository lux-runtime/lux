...
use lux_utils::path::{get_current_dir, LuauFilePath, LuauModulePath};

const LUX_COMMENT_PREFIX: &str = "-->";
...
/**
    ...
    find scripts in `lux` and `.lux` folders if one was not directly found.

    Note that looking in `lux` and `.lux` folders is automatically
...
*/
pub fn discover_script_path_including_lux_dirs(path: impl AsRef<Path>) -> Result<PathBuf> {
    ...
    match discover_script_path(path, false) {
        ...
        Err(e) => {
            ...
            // Otherwise we take a look in relative Lux and .lux
            // directories + the home directory for the current user
            let res = discover_script_path(Path::new("lux").join(path), false)
                .or_else(|_| discover_script_path(Path::new(".lux").join(path), false))
                .or_else(|_| discover_script_path(Path::new("lux").join(path), true))
                .or_else(|_| discover_script_path(Path::new(".lux").join(path), true));

            match res {
                // NOTE: The first error message is generally more
                // descriptive than the ones for the Lux subfolders
                Err(_) => Err(e),
                Ok(path) => Ok(path),
            }
        }
...
pub fn parse_lux_description_from_file(contents: &str) -> Option<String> {
    let mut comment_lines = Vec::new();
    for line in contents.lines() {
        if let Some(stripped) = line.strip_prefix(LUX_COMMENT_PREFIX) {
    ...
    if comment_lines.is_empty() {
        ...
        let shortest_indent = comment_lines.iter().fold(usize::MAX, |acc, line| {
            let first_alphanumeric = line.find(char::is_alphanumeric).unwrap_or(0);
