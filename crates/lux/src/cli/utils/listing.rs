...
use super::files::{discover_script_path, parse_lux_description_from_file};
...
pub async fn find_lux_scripts(in_home_dir: bool) -> Result<Vec<(String, String)>> {
    let base_path = if in_home_dir {
        UserDirs::new()
            .ok_or_else(|| anyhow::anyhow!("Failed to find user home directory"))?
            .home_dir()
            .to_path_buf()
    } else {
        PathBuf::new()
    };
    let mut lux_dir = fs::read_dir(base_path.join("lux")).await;
    if lux_dir.is_err() {
        lux_dir = fs::read_dir(base_path.join(".lux")).await;
    }
    match lux_dir {
        Ok(mut dir) => {
            ...
            let parsed: Vec<_> = files
                ...
                .map(|(entry, _, contents)| {
                    ...
                    let file_name = file_path.file_name().unwrap_or(OsStr::new("")).to_string_lossy();
                    let description = parse_lux_description_from_file(&contents_str);
        ...
        Err(e) if matches!(e.kind(), ErrorKind::NotFound) => {
            bail!("No Lux directory was found.")
        }
        Err(e) => {
            bail!("Failed to read Lux files!\n{e}")
        }
    }
}

pub fn sort_lux_scripts(scripts: Vec<(String, String)>) -> Vec<(String, String)> {
...
pub fn write_lux_scripts_list(buffer: &mut String, scripts: Vec<(String, String)>) -> Result<()> {
