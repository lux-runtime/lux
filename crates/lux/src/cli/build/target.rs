...
pub static CACHE_DIR: LazyLock<PathBuf> = LazyLock::new(|| HOME_DIR.join(".lux").join("target"));

/**
    A target operating system supported by Lux
*/
...
/**
    A target architecture supported by Lux
*/
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/**
    A full target description that Lux supports (OS + Arch)
