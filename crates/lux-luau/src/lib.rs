...
use lux_utils::{jit::JitEnablement, TableBuilder};
...
fn load_source(
    ...
    // Restore JIT to previous state, defaulting to false if not set
    let jit_enabled = lua
        .app_data_ref::<JitEnablement>()
        .map(|j| j.enabled())
        .unwrap_or(false);
    lua.enable_jit(jit_enabled);
