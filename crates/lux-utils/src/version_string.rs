...
/**
    Returns a Lux version string, in the format `Lux x.y.z+luau`.

    The version string passed should be the version of the Lux runtime,
...
*/
#[must_use]
pub fn get_version_string(lux_version: impl AsRef<str>) -> String {
    let lux_version = lux_version.as_ref();

    assert!(!lux_version.is_empty(), "Lux version string is empty");
    match Version::parse(lux_version) {
        Ok(semver) => format!("Lux {semver}+{}", *LUAU_VERSION),
        Err(e) => panic!("Lux version string is not valid semver: {e}"),
    }
}
